...
def build():
    includes = ['static', 'templates', 'favicon.ico', '*.py', '*.sql', 'iphone_dev']
...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        r = run('ls -p -l')
        files = [s[:-1] for s in RE_FILES.split(r) if s.startwith('www-') and s.endswith('/')]
...
def restore2local():
    ...
    sqls = [
        ...
        'grant select, insert, update on awesome.* \'%s\'@localhost\' indentified by \'%s\';' % (db_user, db_password)
    ]
    for sql in sqls:
